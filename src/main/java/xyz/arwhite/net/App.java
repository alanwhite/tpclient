...
public class App 
{
	public static void main( String[] args ) throws IOException, InterruptedException, KeyManagementException, CertificateException, KeyStoreException, NoSuchAlgorithmException
	{
		...
		var stunClient = HttpClient.newBuilder()
				.build();

		HttpRequest stunGet = HttpRequest.newBuilder()
				.GET()
				.uri(URI.create("https://httpbin.org/ip")) 
				.build();

		HttpResponse<String> stunResp = stunClient.send(stunGet, BodyHandlers.ofString());

		System.out.println(stunResp.statusCode());
		System.out.println(stunResp.body());
...
//		proxiedSocketFactory.setProxyHeader(auth);
